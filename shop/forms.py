...
class CartAddProductForm(forms.Form):
    ...
    try:
        PRODUCT_SIZES_CHOICES = [(s.symbol, s.name)
                                 for s in list(Size.objects.all())]
    except:
        pass

    quantity = forms.TypedChoiceField(choices=PRODUCT_QUANTITY_CHOICES,
                                      coerce=int, widget=forms.NumberInput(attrs={'class': 'form-control'}))
    size = forms.ChoiceField(choices=PRODUCT_SIZES_CHOICES, widget=forms.Select(
